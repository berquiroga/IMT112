...
RPM = 180 # Velocidad del motor constante
...
for angulo in angulos_motor:
    ...
    # Si la velocidad angular del eslabón 4 es positiva, la fuerza existe, caso contrario, deja de existir
    if omega4 > 0:
        F_ext = 8000
    else:
        F_ext = 0
    ...
    A[7, 5] = 1
    A[7, 7] = 1
    B[7] = m4 * g + m4 * a4y + F_ext # La carga solamente actúa en un eslabón (no es acción-reacción)
    A[8, 4] = -R34y
    A[8, 5] = R34x
    A[8, 6] = -R14y
    A[8, 7] = R14x
    B[8] = I4 * alpha4 + R34x * F_ext # La carga solamente actúa en un eslabón (no es acción-reacción)

    # Resolver el sistema
    X = np.linalg.solve(A, B)
    torques_motor.append(X[8])
    fuerza_B = np.sqrt(X[4]**2 + X[5]**2)
    fuerzas_B.append(fuerza_B)

# Mostrar resultados y graficar
torque_pico = max(np.abs(torques_motor))
fuerza_B_pico = max(np.abs(fuerzas_B))

# Cálculo de factor de seguridad
factor_seguridad = 1.5
ssy_a36 = 145 * 10**6 # Pa
ssy_4010 = 415 * 10**6 # Pa
diametro = 0.015 # m
area = np.pi * diametro**2 / 4
tau = fuerza_B_pico / area
fs_a36 = ssy_a36 / tau
fs_4010 = ssy_4010 / tau

print(f"TORQUE PICO REQUERIDO: {torque_pico:.2f} N*m")
print(f"FUERZA PICO EJERCIDA EN B: {fuerza_B_pico:.2f} N")
print(f"FACTOR DE SEGURIDAD PARA LA ALEACIÓN ACERO ESTRUCTURAL A36: {fs_a36:.2f}")
print(f"FACTOR DE SEGURIDAD PARA LA ALEACIÓN ACERO ALEADO 4010 BONIFICADO: {fs_4010:.2f}")

# Gráficas
fig, axes = plt.subplots(nrows=2, ncols=2, figsize=(10, 8))
fig.suptitle("Auditoría Dinámica - Bomba de Lodo Asimétrica", fontsize=16, fontweight='bold')
...
# Gráfica para la fuerza total en el pasador B
axes[0, 1].plot(angulos_motor, fuerzas_B, color='blue', linewidth=2)
axes[0, 1].set_title("Fuerza Cortante en Pasador B (F34)")
