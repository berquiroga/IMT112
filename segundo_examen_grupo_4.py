...
L1, L2, L3, L4, L3y = 0.80, 0.2, 0.90, 0.60, 0.4
...
F_ext = 8000 # N, fuerza externa sobre el pasador B
...
for angulo in angulos_motor:
    ...
    a3y = - L2 * np.sin(theta2) * omega2**2 + L2 * np.cos(theta2) * alpha2 - (133/400) * L3y * np.sin(theta3) * omega3**2 + (133/400) * L3y * np.cos(theta3) * alpha3
    ...
    YB = L4 * np.sin(theta4)
    XO4, YO4 = L1, 0.0
    XC = L2 * np.cos(theta2) + (L3/2) * np.cos(theta3)
    YC = L2 * np.sin(theta2) + L3y * np.sin(theta3)
    
    if (YC > 0.20 and angulo >1):
        if (YC < YCs[angulo-1]):
            F_ext = 0
    
    # Centros de Gravedad (Asumiendo a la mitad de las barras)
    XG2, YG2 = XA / 2, YA / 2
    XG3 = XA + (L3/2) * np.cos(theta3+(np.deg2rad(17))) +0.15
    YG3 = YA + (133/400) * L3y * np.sin(theta3+(np.deg2rad(17))) -0.11
    ...
    A[4, 3] = 1
    A[4, 5] = -1
    B[4] = m3 * g + m3 * a3y + F_ext
    A[5, 2] = -R23y
    A[5, 3] = R23x
    A[5, 4] = R43y
    A[5, 5] = -R43x
    B[5] = I3 * alpha3 + F_ext * Rpx
    ...
    YCs.append(YC)
    fuerzas_23.append(np.sqrt(X[2]**2 + X[3]**2))

# Mostrar resultados y graficar
torque_pico = max(np.abs(torques_motor))
esfuerzo = max(np.abs(fuerzas_23)) / area
fs = Ssy / esfuerzo
print(f"Coordenadas x', y' optimas del CG: {XG3:.2f}, {YG3:.2f} m")
print(f"TORQUE PICO REQUERIDO: {torque_pico:.2f} N*m")
print(f"FACTOR DE SEGURIDAD: {fs:.2f}")

if fs > 1.5:
    print("El diseño es seguro.")
